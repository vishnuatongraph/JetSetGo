...
import { View, Image, Text, SafeAreaView, Pressable, FlatList } from 'react-native';
...
export default function Flights() {
    ...
    const getSelectData = useMemo(() => {
        return [...new Set(searchedFlights.map(flight => flight?.displayData?.airlines[0]?.airlineName) || [])]
    }, [searchedFlights])
    ...
    useEffect(() => {
        setFilteredFlight(handleSortByPrice(searchedFlights))
    }, [priceOrder])
            ...
            <View style={styles.container}>
                <View style={styles.header}>
                    <Pressable style={{ flexDirection: 'row' }} onPress={() => navigation.goBack()}>
                        <Image source={Back} alt='back' style={{ height: 20, width: 20, marginRight: 10 }} />
                        <Text>Back</Text>
                    </Pressable>
                ...
                <View style={styles.headingContainer}>
                    <Select
                        ...
                        onValueChange={itemValue => {
                            ...
                            const filteredFlightsByService = searchedFlights?.filter(item => item?.displayData?.airlines[0]?.airlineName === itemValue);
                            const sortedFlightsByPrice = handleSortByPrice(filteredFlightsByService);
                            setFilteredFlight(sortedFlightsByPrice);
                        }}
                    >
